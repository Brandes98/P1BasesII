...
@pytest.fixture
def app_context():
    app = create_app({'TESTING': True})  # Ensure this uses your Flask app's factory function
...
def test_get_users(app_context):
    result = db.get_users()
    assert result is not None
    assert result[1] == 200  # Assuming get_users returns a tuple with (response, status_code)
    assert "status" in json.loads(result[0].data)  # Check that the response has a 'status' key
