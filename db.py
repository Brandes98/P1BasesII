...
class Database:
    ...
    def get_users(self):
        ...
        cursor = self.conn.cursor()
        cursor.execute("""
                SELECT U.id, U.Nombre, R.Nombre AS RoleName, U.Correo, OBTENER_GENERO(U.Genero) AS Gender, P.Nombre AS Country
                FROM Usuarios AS U
        ...
        if users:
            ...
            self.redis_client.setex(cache_key, 3600, json.dumps(users))  
