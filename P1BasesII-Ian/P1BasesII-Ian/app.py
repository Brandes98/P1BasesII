...
def get_encuestados():
    try:
        respondents = appService.get_encuestados()
...
def get_encuestado(id):
    try:
        respondent = appService.get_encuestado(id)
        return str(respondent)
    except Exception as e:
        return jsonify({"error": str(e)}), 500
@app.route("/respondents/<int:id>", methods=["PUT"])
def actualizar_encuestado(id, data):
    try:
        respondent = appService.actualizar_encuestado(id, data)
        return str(respondent)
    except Exception as e:
        return jsonify({"error": str(e)}), 500
